...
// https://github.com/dotnet/aspnetcore/blob/main/src/Components/Web/src/Forms/InputFile/BrowserFileStream.cs
...
namespace Blazor.Cherrydown.FileUpload
{
    internal sealed class BrowserFileStream : Stream
    {
        ...
        private readonly ElementReference _cherrydownElement;
        ...
        public BrowserFileStream(
            JsInterop jsInterop,
            ElementReference cherrydownElement,
            BrowserFile file,
            long maxAllowedSize,
            CancellationToken cancellationToken)
        {
            _jsInterop = jsInterop;
            _cherrydownElement = cherrydownElement;
            _file = file;
        ...
        private async Task<Stream> OpenReadStreamAsync(CancellationToken cancellationToken)
        {
            ...
            var module = await _jsInterop.ModuleTask.Value;
            _jsStreamReference = await module.InvokeAsync<IJSStreamReference>(
                "readFileData",
                cancellationToken,
                _cherrydownElement,
                _file.Id);
